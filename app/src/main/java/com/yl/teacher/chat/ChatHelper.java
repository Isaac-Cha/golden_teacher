...
import com.yl.teacher.chat.db.DemoDBManager;
import com.yl.teacher.chat.db.InviteMessgeDao;
import com.yl.teacher.chat.db.UserDao;
import com.yl.teacher.chat.domain.InviteMessage;
import com.yl.teacher.chat.domain.InviteMessage.InviteMesageStatus;
import com.yl.teacher.chat.domain.RobotUser;
import com.yl.teacher.chat.parse.UserProfileManager;
import com.yl.teacher.chat.receiver.CallReceiver;
import com.yl.teacher.chat.ui.activity.ChatActivity;
import com.yl.teacher.chat.util.PreferenceManager;
import com.yl.teacher.util.AppUtils;
import com.yl.teacher.util.Static;
import com.yl.teacher.util.UiUtils;
import com.yl.teacher.view.LoginActivity;
...
public class ChatHelper {
    ...
    private EMOptions initChatOptions() {
        ...
        options.setMipushConfig(Static.ML_MI_APP_ID, Static.ML_MI_APP_KEY);
        // 设置集成华为推送的appid
        options.setHuaweiPushAppId(Static.ML_HUAWEI_APP_ID);
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
    ...
    protected void onConnectionConflict() {
        Intent mIntent = new Intent(appContext, LoginActivity.class);
        mIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        mIntent.putExtra(Static.LOGIN_TYPE, Static.ACCOUNT_CONFLICT);
        AppUtils.loginOut(appContext, mIntent);
    }
    ...
    protected void onCurrentAccountRemoved() {
        Intent mIntent = new Intent(appContext, LoginActivity.class);
        mIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        mIntent.putExtra(Static.LOGIN_TYPE, Static.ACCOUNT_REMOVED);
        AppUtils.loginOut(appContext, mIntent);
    }
    ...
    public EaseUser getUserInfo(String hxId) {
        ...
        // 获取不在好友列表里的群成员具体信息，即陌生人信息
        if (easeUser == null && getRobotList() != null) {
            easeUser = getRobotList().get(hxId);
        }
        ...
        if (easeUser == null) {
            ...
            if (TextUtils.isEmpty(easeUser.getNick())) { // 如果名字为空，则显示环信号码
                easeUser.setNick(emmGroup.nickname);
            }
        }
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    LogUtil.w("userPic: " + userPic);
                    LogUtil.w("chatType: " + chatType);
                    LogUtil.w("ChatType.GroupChat: " + ChatType.GroupChat);
                    // ******************扩展信息处理完成**********************

                    EMLog.d(TAG, "onMessageReceived id : " + message.getMsgId());
                    // in background, do not refresh UI, notify it in notification bar
                    if (!easeUI.hasForegroundActivies() && chatType == ChatType.Chat) {
